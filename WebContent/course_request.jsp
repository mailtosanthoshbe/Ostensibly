		...
		<table class="stud" width="90%"
			...
			<tr>
				...
				<%
					...
					switch (rs.getInt(8)) {
						case 2:
							status = "Allocated but Payment pending";
							break;
						case 3:
							status = "Allocated but Payment pending";
							break;
					...
					if(rs.getString(7) == null){
						status = "No staff available.";
					}
				%>
				<td><a
					href="${pageContext.request.contextPath}/CourseDetails?action=notpaid&allotid=<%=rs.getInt(1)%>&sid=<%=rs.getInt(6)%>"><%=status%></a></td>
			</tr>
			<%
				}
				} catch (Exception ex) {
